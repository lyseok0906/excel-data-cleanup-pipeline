...
**Full PSL — RESOLVED (2026-09-17, 이전 문구는 아래에 이력으로 남긴다).** 이 프로젝트의 이전 실행 환경들은 PyPI/publicsuffix.org 등 외부 네트워크에 접근할 수 없어 `tldextract`를 설치/사용할 수 없었다. 사용자가 본인 로컬 Windows Python(>=3.10) 환경에서 `pip install tldextract==5.3.2` 설치·import를 직접 확인한 뒤, `validation/scripts/domain_utils.py`를 `tldextract`의 전체 Public Suffix List를 사용하는 구현으로 교체했다: 모듈 레벨 singleton `TLDExtract(suffix_list_urls=(), cache_dir=None, fallback_to_snapshot=True, include_psl_private_domains=True)`을 사용하며, `suffix_list_urls=()`/`cache_dir=None`은 런타임 네트워크 접근을 완전히 차단하고(`fallback_to_snapshot=True`가 tldextract 내장 스냅샷을 사용하게 함), `include_psl_private_domains=True`는 이 리서치의 명시적 정책 결정이다 — blogspot.com, github.io 같은 PSL PRIVATE 항목을 그 자체로 독립적인 registrable suffix로 취급해, `foo.blogspot.com`과 `bar.blogspot.com`처럼 같은 호스팅 플랫폼 위의 서로 다른 사이트를 서로 다른 registrable domain으로 정확히 구분한다. `registrable_domain()`은 `result.top_domain_under_public_suffix`를 사용한다. 의존성은 `validation/requirements.txt`에 `tldextract==5.3.2`로 pin됐다. `is_clean_root_form()`도 이번에 강화되어, 표면적 포맷(scheme/www/path/대소문자) 검사에 더해 `raw == registrable_domain(raw)`까지 만족해야 True를 반환한다(예: `blog.example.com`은 이제 False — 이전에는 www. 접두어가 없다는 이유만으로 통과했었다). `wave_coordinator.run_wave()`도 candidate raw 문자열을 RESERVE 이전에 `registrable_domain()`으로 canonicalize하도록 수정됐다(이전에는 문서에만 명시되고 실제로는 raw 문자열이 그대로 저장됐음).

*(이전 문구, 이력 보존용)* — 이전 실행 환경들에서는 co.uk, com.au, co.jp 등 흔히 등장하는 multi-label suffix를 수십 개 curated 목록으로 내장하고, 목록에 없는 suffix는 "마지막 두 라벨" 일반 규칙으로 처리하는 경량 구현을 썼다. 알려진 한계는 목록에 없는 드문 multi-label suffix가 잘못 판정될 수 있다는 것과, PRIVATE PSL 항목(blogspot.com 등)을 전혀 구분하지 못해 서로 다른 사이트가 잘못 collapse된다는 것이었다 — 둘 다 이번 교체로 해소됐다.
...
## 7-F. Full PSL blocker 해결 (완료, 2026-09-17, commit ddcdda7 후속)

GPT가 commit `ddcdda7`(실제로는 `13a383daa4...`)을 코드 수준까지 검토한 뒤 남아 있던 마지막 production blocker(Section 7-E Item 7)를 해결하는 라운드. **이번 라운드도 새 사이트 리서치 0건, Study B 미착수, Study A 950개 미실행, 기존 50-site 조사 데이터의 내용(값) 자체는 변경하지 않았다.** 이 라운드는 Full PSL/domain canonicalization 코드만 수정했다.

1. **환경 제약 해소 경위.** 클라우드 샌드박스(PyPI 접근 여전히 차단)와 `device_bash`(2026-09-08 Windows 업데이트 이슈로 여전히 기동 실패, 이번 라운드에도 재확인)는 변함없이 막혀 있다. 하지만 사용자가 본인 로컬 Windows Python(>=3.10) 환경에서 `pip install tldextract==5.3.2` 설치와 import를 직접 확인해 주어, 이 커밋에서 `tldextract` 기반 구현으로 교체할 수 있게 됐다. 단, 이 세션은 여전히 tldextract를 직접 실행할 수 없으므로 아래 코드 변경은 이 세션에서 유닛 테스트/파이프라인 재실행으로 자체 검증되지 못했다 — 실행/검증은 사용자가 본인 로컬 환경에서 수행하고 그 결과를 별도로 확인받는 절차로 대체됐다(자세한 내용은 최종 보고 참고).
2. **의존성 신설.** 리포지토리에 기존 Python 의존성 파일이 전혀 없었음을 확인한 뒤(`requirements.txt`/`pyproject.toml`/`setup.py`/`Pipfile` 전부 부재), `validation/requirements.txt`를 신설해 `tldextract==5.3.2`를 pin하고 Python >=3.10 요구사항을 명시했다(`validation/scripts/README.md`에도 동일하게 문서화).
3. **`domain_utils.py` 전면 교체.** `COMMON_MULTI_LABEL_SUFFIXES` 수동 테이블과 "마지막 두 라벨" fallback 방식을 제거하고, 모듈 레벨 singleton `TLDExtract(suffix_list_urls=(), cache_dir=None, fallback_to_snapshot=True, include_psl_private_domains=True)`로 교체했다. `include_psl_private_domains=True`는 이 리서치의 명시적 정책 결정으로, blogspot.com/github.io 같은 PSL PRIVATE 항목을 독립적인 registrable suffix로 취급해 같은 호스팅 플랫폼 위의 서로 다른 사이트를 서로 다른 도메인으로 정확히 구분한다(`example.co.uk` 같은 PUBLIC suffix 처리는 기존과 동일). `registrable_domain()`은 `result.top_domain_under_public_suffix`를 사용한다. tldextract가 import되지 않으면 명확한 설명과 함께 `ImportError`를 raise하도록 해 Full PSL이 이제 optional이 아니라 required dependency임을 코드 레벨에서 강제한다.
4. **`is_clean_root_form()` 강화.** 표면적 포맷 검사(scheme 없음/www. 없음/path 없음/소문자)에 더해 `raw == registrable_domain(raw)`까지 만족해야 True를 반환하도록 강화했다. 이제 `blog.example.com`은 False를 반환한다(이전에는 www. 접두어가 없다는 이유만으로 True였음). `foo.blogspot.com`은 PRIVATE suffix가 독립 사이트 정체성으로 취급되므로 여전히 True다.
5. **`wave_coordinator.run_wave()` canonicalize 실제 구현.** 이전 라운드까지는 문서에만 명시되고 실제로는 raw candidate 문자열이 그대로 저장됐다. 이번에 RESERVE/duplicate-check/research 디스패치 이전에 모든 candidate를 `domain_utils.registrable_domain()`으로 canonicalize하고, canonicalize 결과가 비어있는 candidate가 하나라도 있으면 어떤 research도 시작하지 않고 즉시 `WaveAbortedError`로 중단하도록 했다. 같은 wave 내에서 canonicalize 후 중복되는 candidate도 이 단계에서 de-duplicate한다. 예: `https://www.example.com/path`는 registry에 `example.com`으로만 저장된다.
6. **테스트 전환 — desired-behavior 정상 테스트로 교체.** `@unittest.expectedFailure`로 표시돼 있던 `test_full_psl_blogspot_subdomains_should_be_distinct`를 제거하고, 다음 7건의 일반(비-expectedFailure) 테스트로 교체했다: blogspot 서브도메인 구분, github.io 서브도메인 구분, 기존 plain domain(`example.com`/`example.co.uk`) 동작 유지 확인, `is_clean_root_form()`이 서브도메인을 거부하는지, PRIVATE PSL 항목(`foo.blogspot.com`)을 허용하는지, plain root를 허용하는지, scheme/www/path가 섞인 문자열을 거부하는지. 여기에 더해 `TestWaveTransactionOrdering`에 URL candidate가 registrable domain으로 canonicalize되어 저장되는지 확인하는 테스트를 1건 추가했다.
7. **문서 동기화.** Protocol Section 6-3을 "Full PSL — RESOLVED"로 갱신했고(이전 문구는 이력으로 보존), 이 Section 7-F를 신설해 라운드 완료를 기록했다. `validation/scripts/README.md`의 상단 요약, `domain_utils.py` 항목, `wave_coordinator.py` 항목, `test_validation_tooling.py` 항목을 모두 갱신했다.
8. 이번 라운드의 모든 변경은 커밋 1건으로 묶고 push하지 않는다(사용자 지시). **950개 확장은 여전히 시작하지 않았다.**

**중요 — 이번 라운드에서 이 세션이 직접 확인하지 못한 것:** 이 세션은 tldextract를 실행할 수 있는 환경(클라우드 샌드박스, `device_bash` 둘 다)이 없어 테스트 스위트 재실행, Gate 재실행, registry 재생성, wave demo 재실행을 이 세션에서 직접 수행하지 못했다. 이 검증은 사용자가 본인 로컬 환경에서 실행한 결과를 근거로 별도로 확인됐다(결과는 최종 보고에 기재). **기존 50-site 데이터(및 147행 registry)에 Full PSL 적용으로 새로운 registrable-domain collision이 발견될 경우, 이 라운드에서는 임의로 수정하지 않고 STOP 후 보고하기로 했다** — 실제로 그런 collision이 있었는지 여부도 최종 보고에 기재한다.
